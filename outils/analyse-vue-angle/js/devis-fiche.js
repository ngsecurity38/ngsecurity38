...
<table>
  ...
  ? `Total des options chiffrées, à ajouter au prix ci-dessus`
  : `Sous-total ${echapper(lot.titre.toLowerCase())}`}</td>
      <td class="n">${euros(lot.total)}</td></tr>
  </tbody>
</table>
${opt && lot.sansPrix ? `<p class="det">${lot.sansPrix === 1
  ? 'Une option reste à chiffrer et vous sera communiquée'
  : `${lot.sansPrix} options restent à chiffrer et vous seront communiquées`}
  avant signature. ${lot.sansPrix === 1 ? 'Elle n\'est' : 'Elles ne sont'} pas
  compté${lot.sansPrix === 1 ? 'e' : 'es'} dans le total ci-dessus.</p>` : ''}`;
  }).join('\n');
}
